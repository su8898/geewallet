...
and internal ActiveChannel =
    ...
    member internal self.SendHtlcPayment (amount: LNMoney)
        ...
        let addHtlcMsgRes, channelAfterAddHtlcPayment =
            let channelCmd = ChannelCommand.AddHTLC { Amount = amount; PaymentHash = (PaymentHash.PaymentHash paymentPreimage); Expiry = BlockHeight blockHeight; Onion = onionPacket.Packet; Upstream=None; Origin = None; CurrentHeight = BlockHeight.Zero }
            channel.ExecuteCommand channelCmd <| function
